...
# Prefer tflite_runtime; fall back to TF if available
try:
...
# COCO SSD model (the classic "detect.tflite") + 80-class labelmap
OBJ_MODEL = MODEL_DIR / "detect.tflite"
LBL_PATH  = MODEL_DIR / "labelmap.txt"     # 80-class list (COCO-80 style)
...
def _ensure_capture(width=1280, height=720, fps=30):
    """Open the global VideoCapture if closed; prefer MJPG."""
    global _cap
    with _cap_lock:
...
def _read_frame() -> Optional[np.ndarray]:
    """Read a frame; auto-reopen once on failure."""
    global _cap
    _ensure_capture()
    cap = _cap
    if cap is None:
...
# ---------- Helpers ----------
def _norm(s: str) -> str:
    """Normalize label for matching: lowercase and strip non-alnum."""
    return "".join(ch for ch in s.lower() if ch.isalnum())

# ---------- Labels / SSD loader ----------
def _load_labels(p: pathlib.Path) -> List[str]:
    """Supports either 'index label' or 'label' per line; trims blanks."""
    try:
        out = []
...
def _load_ssd(model_path: pathlib.Path):
    """Load SSD TFLite and remember input dtype/shape and output indices."""
    ...
    idtype = iinfo["dtype"]  # np.uint8 or np.float32
    return {"itp": itp, "iinfo": iinfo, "oinfo": oinfo,
            "bix": bix, "cix": cix, "six": six, "nix": nix,
            "ih": ih, "iw": iw, "idtype": idtype}

# ---------- Detection tweaks ----------
OBJ_SCORE_THRESH = 0.45      # raise threshold to reduce junk
OBJ_NMS_IOU      = 0.45

def _nms(dets, iou_thr=OBJ_NMS_IOU):
    """dets: (x,y,w,h,cid,score,name)"""
    if not dets: return []
...
class Runtime:
    def __init__(self):
        ...
        self.allow: List[str] = []          # user-visible list (raw strings)
        self.allow_norm: set[str] = set()   # normalized set used for matching
        self.fps = 0.0
        self.faces = 0
        self.objects = 0
        ...
        self.obj_rt = None
        self.obj_lock = threading.Lock()    # TFLite is not thread-safe

        self.profiles: List[Dict] = []      # simple face “recognition”
        self._fps_count = 0
        self._t0 = time.time()

    # profiles (placeholder recognition)
    @staticmethod
    ...
    def infer_objects(self, frame_bgr: np.ndarray):
        ...
        # classic detect.tflite is uint8; float models expect [0,1]
        if idtype == np.uint8:
        ...
        for i in range(count):
            ...
            if sc < OBJ_SCORE_THRESH:
                continue
            cid = int(classes[i])
            # robust label lookup against the 80-class list
            name = OBJ_LABELS[cid] if 0 <= cid < len(OBJ_LABELS) else str(cid)

            # Whitelist mode: if allow_norm has entries, only pass those
...
def api_status(ctx: dict, **_) -> Dict[str, object]:
    with RT.lock:
        return {
            ...
            "faces": RT.faces,
            "objects": RT.objects,
            "overlay": RT.overlay_enabled,        # legacy
            "allow": list(RT.allow),              # legacy
            "vision": {                           # what the FE reads
                "enabled": RT.overlay_enabled,
                "allow": list(RT.allow),
            },
        }
...
def api_allow(ctx: dict, allow: List[str] | None = None, **_) -> Dict[str, object]:
    """Set the allow list. Empty/None => detect ALL classes."""
    if allow is None:
        return {"ok": True, "allow": list(RT.allow)}

    # Only keep labels that exist in the model (case/spacing agnostic)
    ...
    for s in allow:
        ...
        if n and n in labelset_norm:
            cleaned.append(raw)  # keep original text for UI chips
            normed.add(n)

    with RT.lock:
        RT.allow = cleaned                 # for UI echo
        RT.allow_norm = normed             # for inference
...
# Convenience alias used by index.py when it calls "cam"
